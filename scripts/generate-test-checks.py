...
def workspace_is_dirty(path):
    ...
    if "not a git repository" in result.stderr and not has_git_metadata(path):
        return False
...
def main():
    ...
    parser.add_argument(
        "--source",
        type=str,
        help="Place each generated CHECK segment before the corresponding segment in "
        "this source file. Segments are selected by --source_delim_regex.",
    )
    parser.add_argument(
        "--source_delim_regex",
        type=str,
        default="func @",
        help="Regex matching source lines that begin placement segments (default: 'func @').",
    )
    parser.add_argument(
        "--starts_from_scope",
        type=int,
    ...
    try:
        ...
        if source_segments:
            if len(output_segments) != len(source_segments):
                print(
                    "error: generated "
                    f"{len(output_segments)} CHECK segment(s) for "
...
class GenerateTestChecksTests(unittest.TestCase):
    """Exercise command-line output, source preservation, and failure behavior."""
    ...
    def test_named_output_matches_stdout(self):
        """Named and bare output options retain the result written to stdout."""
        input_path = self.write_file(
            "input.llzk",
            "module {\n  func.func @main() {\n  }\n}\n",
        )
        ...
        bare_code, bare_output, bare_error = self.invoke(
        ...
        self.assertEqual(bare_error, "")
        self.assertEqual(named_error, "")
        self.assertEqual(bare_output, stdout_output)
        self.assertEqual(named_output.read_text(encoding="utf-8"), stdout_output)

    def test_bare_output_conflicts_with_inplace(self):
    ...
    def test_segment_mismatch_preserves_source(self):
        ...
        self.assertEqual(exit_code, 1)
        self.assertIn("CHECK segment(s) for", error)
        self.assertEqual(source.read_bytes(), before)
    ...
    def test_inplace_uses_real_git_status_and_detects_hidden_untracked_files(self):
        """A hidden untracked file blocks in-place update until the worktree is clean."""
