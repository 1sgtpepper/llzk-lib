...
def process_source_lines(source_lines, note, args):
    ...
    while line_index < len(source_lines):
        ...
        if source_lines[line_index : line_index + len(note_lines)] == note_lines:
            line_index += len(note_lines)
            if line_index < len(source_lines) and source_lines[line_index] == "":
                line_index += 1
            continue
        # Also remove the one-line note emitted by older versions.
        if source_lines[line_index] == note_lines[0]:
            line_index += 1
            if line_index < len(source_lines) and source_lines[line_index] == "":
                line_index += 1
            continue
    ...
    if args.source:
        with open(args.source, "r") as source:
            source_segments = process_source_lines(
                [l.rstrip() for l in source], autogenerated_note, args
            )
...
class GenerateTestChecksTests(unittest.TestCase):
    ...
    def test_custom_prefix_removes_only_selected_directives(self):
        """Regeneration removes stale selected-prefix checks but preserves other checks."""
        source = self.write_file(
            "source.llzk",
            "module {\n"
            "  // ALT: stale selected check\n"
        ...
        self.assertEqual(exit_code, 0)
        self.assertEqual(error, "")
        self.assertNotIn("ALT: stale selected check", contents)
        self.assertIn("// CHECK: preserve default check", contents)
        self.assertIn("// ALT-", contents)
    ...
    def test_conflicting_output_modes_are_rejected(self):
        ...
        for options in (
            ...
            with self.subTest(options=options):
                exit_code, _, error = self.invoke(*options)
