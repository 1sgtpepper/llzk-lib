...
def process_source_lines(source_lines, note, args):
    ...
    while line_index < len(source_lines):
        # Remove a complete previous note and its separating blank line.
        if [
            line_without_ending(line)
            for line in source_lines[line_index : line_index + len(note_lines)]
        ] == note_lines:
            line_index += len(note_lines)
            if (
                ...
                line_index += 1
            continue
        # Also remove the one-line note emitted by older versions.
        if line_without_ending(source_lines[line_index]) == note_lines[0]:
            line_index += 1
            if (
                line_index < len(source_lines)
                and line_without_ending(source_lines[line_index]) == ""
...
def main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Allow in-place updates in a dirty Git worktree.",
    )
    parser.add_argument(
        "--variable_names",
        type=str,
    ...
    if args.inplace:
        ...
        if not args.allow_dirty:
            try:
                paths_to_check = [source_path]
                if source_argument_path != source_path:
                    paths_to_check.append(source_argument_path)
                dirty = any(workspace_is_dirty(path) for path in paths_to_check)
        ...
        if not os.access(source_path, os.W_OK):
            close_input()
            print(f"error: source file is not writable: {args.source}", file=sys.stderr)
            return 1
    ...
    try:
        ...
        if args.inplace:
            ...
            os.chmod(temporary_source, stat.S_IMODE(os.stat(source_path).st_mode))
        ...
        if args.inplace:
            ...
            if temporary_source is not None:
                try:
                    ...
                    cleanup_errors.append(("remove temporary source file", error))
...
class GenerateTestChecksTests(unittest.TestCase):
    """Exercise the command-line utility's file and failure contracts."""
    ...
    def assert_no_temporary_source(self, source):
    ...
    def test_success_returns_zero_and_generates_checks(self):
        """A normal invocation returns success and emits the generator note and checks."""
        input_path = self.write_file(
            "input.llzk",
            "module {\n  func.func @main() {\n  }\n}\n",
        )

        exit_code, output, error = self.invoke(str(input_path))

        self.assertEqual(exit_code, 0)
        self.assertEqual(error, "")
        self.assertIn("NOTE: Assertions have been autogenerated", output)
        self.assertIn("// CHECK-LABEL:", output)

    def test_named_output_matches_stdout(self):
        """Named output retains the same generated result as stdout output."""
        input_path = self.write_file(
            "input.llzk",
            "module {\n  func.func @main() {\n  }\n}\n",
        )
        ...
        stdout_code, stdout_output, stdout_error = self.invoke(
            "-o", "-", str(input_path)
        )
    ...
    def test_bare_output_requires_a_path(self):
        """A bare output option cannot silently become an in-place update."""
        ...
        self.assertIn("argument -o/--output: expected one argument", error)
    ...
    def test_inplace_preserves_source_bytes_across_repeated_runs(self):
        ...
        self.assertEqual(first_contents.count(b"NOTE: Assertions have been autogenerated"), 1)
        self.assertNotIn(b"CHECK-LABEL: stale", first_contents)
        self.assertIn(b"// CHECK-LABEL:", first_contents)
        self.assertIn(b"#map = affine_map<(d0) -> (d0)>  \r\n", first_contents)
        self.assertIn(b"\t\r\n", first_contents)
        self.assertIn(b"module {  \r\n", first_contents)
        self.assertIn(b"  // source comment with trailing spaces   \r\n", first_contents)
        self.assertIn(b"  }  \r\n", first_contents)

    def test_inplace_is_idempotent_and_preserves_source_and_permissions(self):
        """Successful in-place updates preserve source text and permission bits across runs."""
    ...
    def test_custom_prefix_removes_only_selected_directives(self):
        """Custom-prefix regeneration is complete, selective, and idempotent."""
        source = self.write_file(
            "source.llzk",
            "#map = affine_map<(d0) -> (d0)>\n"
            "module {\n"
        ...
        for stale in ("stale plain", "stale next", "stale label", "stale same"):
    ...
    def test_conflicting_output_modes_are_rejected(self):
        """Both option orders reject combining in-place and named output modes."""
        source = self.write_file(
            "source.llzk",
        ...
        before = source.read_bytes()
        for options in (
            ("--inplace", "--output", str(self.root / "output.llzk")),
            ("--output", str(self.root / "output.llzk"), "--inplace"),
        ):
            with self.subTest(options=options):
                exit_code, _, error = self.invoke(
                    "--source", str(source), *options, str(input_path)
                )

                self.assertEqual(exit_code, 2)
                self.assertIn(
                    "--inplace cannot be used with --output", error
                )
                self.assertEqual(source.read_bytes(), before)
                self.assertFalse((self.root / "output.llzk").exists())
    ...
    def test_legacy_one_line_note_is_removed(self):
        ...
        self.assertEqual(error, "")
        self.assertEqual(
            contents.count("NOTE: Assertions have been autogenerated"), 1
        )
        self.assertNotIn("CHECK-LABEL: stale", contents)
        self.assertIn("The script is designed to make adding checks", contents)
    ...
    def test_replacement_failure_preserves_source_and_cleans_temporary_file(self):
        ...
        def fail_replace(temporary, target):
            ...
            replacement_arguments.append((temporary, target))
    ...
    def test_unwritable_source_is_rejected_before_output(self):
        """An unwritable in-place source is rejected before creating temporary output."""
        source = self.write_file(
            "source.llzk",
            "module {\n  func.func @main() {\n  }\n}\n",
        )
        ...
        self.assertEqual(exit_code, 1)
        self.assertIn("source file is not writable", error)
        self.assertEqual(source.read_text(encoding="utf-8"), before)
        self.assert_no_temporary_source(source)
    ...
    def test_symlink_inplace_updates_target_without_replacing_link(self):
        ...
        self.assertEqual(
            ...
            [str(target), str(link.absolute())],
        )
        self.assertIn(
            "NOTE: Assertions have been autogenerated", target.read_text(encoding="utf-8")
        )

    def test_symlink_dirty_check_reports_second_path_and_preserves_target(self):
        """A dirty symlink path is checked after its clean target and blocks replacement."""
        target = self.write_file(
            "target/source.llzk",
            "module {\n  func.func @main() {\n  }\n}\n",
        )
        ...
        dirty_check = mock.Mock(side_effect=[False, True])
        with mock.patch.dict(globals(), {"workspace_is_dirty": dirty_check}):
            exit_code, _, error = self.invoke(
                "--source",
                str(link),
        ...
        self.assertEqual(exit_code, 1)
        self.assertIn("warning: refusing to modify a dirty Git worktree", error)
    ...
    def test_workspace_is_dirty_includes_untracked_status(self):
        """Git status is queried with all untracked files enabled."""
        source = self.write_file("repo/source.llzk", "module {}\n")
        completed = subprocess.CompletedProcess(
            args=[], returncode=0, stdout="?? .hidden", stderr=""
        )

        with mock.patch.object(subprocess, "run", return_value=completed) as run:
            self.assertTrue(workspace_is_dirty(source))
    ...
    def test_inplace_uses_real_git_status_and_detects_hidden_untracked_files(self):
        """The composed guard blocks hidden untracked files and permits a clean worktree."""
        ...
        blocked_code, _, blocked_error = self.invoke(
            "--source", str(source), "-i", str(input_path)
        )
        blocked_contents = source.read_bytes()
        hidden.unlink()
        ...
        self.assertIn("refusing to modify a dirty Git worktree", blocked_error)
        self.assertEqual(blocked_contents, before)
    ...
    def test_workspace_is_dirty_handles_non_repository_and_status_failure(self):
        ...
        with mock.patch.object(
            ...
            self.assertFalse(workspace_is_dirty(source))
