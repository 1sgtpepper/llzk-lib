...
public:
  ClonedBodyConstReadOpPattern(
      ...
      std::optional<Diagnostic> &instantiationFailureDiagnostic
  )
      ...
      : super(converter, ctx, /*patternBenefit=*/1, paramNameToInstantiatedValue),
        diagnostics(instantiationDiagnostics), failureDiagnostic(instantiationFailureDiagnostic) {}
  ...
  LogicalResult handleRewrite(
    ...
    if (failed(materializedType)) {
      if (!failureDiagnostic) {
        failureDiagnostic.emplace(op.getLoc(), DiagnosticSeverity::Error);
        failureDiagnostic->append(
            "'", op->getName(), "' op cannot materialize constant with type ", a.getType(),
            " as expected type ", ty
        );
      }
      return failure();
...
static FailureOr<std::optional<TemplateExprOp>> cloneDeferredExpr(
    ...
    SmallVector<Diagnostic> &diagnostics, std::optional<Diagnostic> &failureDiagnostic
) {
  ...
  patterns.add<ClonedBodyConstReadOpPattern>(
      tyConv, ctx, paramNameToConcrete, diagnostics, failureDiagnostic
  );
    ...
    if (auto constReadOp = llvm::dyn_cast<ConstReadOp>(bodyOp)) {
      ...
      if (auto feltTy = llvm::dyn_cast<FeltType>(constReadOp.getType())) {
        if (auto intAttr = llvm::dyn_cast<IntegerAttr>(val)) {
          ...
          if (failed(materializedType)) {
            constReadOp.emitOpError().append(
                "cannot materialize constant with type ", feltAttr.getType(), " as expected type ",
                feltTy, " while evaluating template expression"
            );
            return failure();
          }
  ...
  FailureOr<StructType> genClone(StructType typeAtCaller, ArrayRef<Attribute> typeAtCallerParams) {
    ...
    if (remainingNames.empty()) { // FULL INSTANTIATION CASE
      ...
      for (TemplateExprOp exprOp : deferredExprs) {
        FailureOr<std::optional<TemplateExprOp>> clonedExpr = cloneDeferredExpr(
            exprOp, paramNameToConcrete, deferredExprDiagnostics, failureDiagnostic
        );
        if (failed(clonedExpr)) {
          if (failureDiagnostic) {
            ctx->getDiagEngine().emit(std::move(*failureDiagnostic));
          }
          newTemplate->destroy();
          newStruct->destroy();
          return failure();
        }
        if (!clonedExpr->has_value()) {
          newTemplate->destroy();
          newStruct->destroy();
          return failure();
        }
...
static LogicalResult applyBodyConversions(
  ...
  if (failed(applyFullConversion(newFunc, target, std::move(bodyPatterns)))) {
    if (failureDiagnostic) {
      failureDiagnostic->attachNote(op.getLoc()).append("while instantiating this call");
      ctx->getDiagEngine().emit(std::move(*failureDiagnostic));
    }
    return failure();
  }
    ...
    if (std::optional<TemplateOp> cached =
      ...
      for (TemplateExprOp exprOp : deferredExprs) {
        ...
        if (failed(clonedExpr)) {
          if (failureDiagnostic) {
            failureDiagnostic->attachNote(op.getLoc()).append("while instantiating this call");
            op.getContext()->getDiagEngine().emit(std::move(*failureDiagnostic));
          }
          newTemplate->destroy();
          return failure();
        }
