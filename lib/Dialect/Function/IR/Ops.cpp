...
struct KnownTargetVerifier : public CallOpVerifier {
  ...
  LogicalResult verifyTemplateParams() override {
    ...
    if (TemplateOp parent = getParentOfType<TemplateOp>(tgtOp)) {
      return llzk::verifyKnownTargetTemplateParams(
          callOp->getOperation(), callOp->getTypeSignature(), tgtType, includeSymNames,
          tgt.getSymName(), parent.getSymName(), callOp->getTemplateParamsAttr(),
