...
- `array.type<N x E>`: Aggregate type with indexed [pseudo-homogeneous](\ref pseudo-homogeneous) elements. Element type cannot be another array type, instead multi-dimensional arrays are specified with a comma-separated list of dimension sizes. Each dimension size can be an integer literal, a flat reference to an index-typed `poly.param` or `poly.expr` binding, a fully-qualified reference to an index-typed constant global, or a single-result [affine map](https://mlir.llvm.org/docs/Dialects/Affine/#polyhedral-structures).
- `struct.type<@Name<[...]>>`: Aggregate type declared by `struct.def @Name`. For a definition in a `poly.template`, optional instantiation arguments correspond to its `poly.param` declarations in order. Arguments may be integers, felt constants, symbols, types for `poly.tvar` parameters, or [affine maps](https://mlir.llvm.org/docs/Dialects/Affine/#polyhedral-structures). Concrete arguments must satisfy the declared restriction. A fieldless felt constant or integer can satisfy a fielded restriction; a conflicting explicit field is rejected. A flat symbol must name a compatible `poly.param` or `poly.expr` binding; a constant global must be fully qualified and have a compatible type. Type arguments resolve nested `struct.type` and `array.type` references at the use site.
...
- `poly.tvar<@N>`: Placeholder type bound to `poly.param @N` in an enclosing `poly.template`. A `!poly.tvar` restriction limits that parameter to type arguments and enables type inference.
...
- A templated `function.call` or `verif.include` may omit arguments only when every parameter appears in the target signature and its inferred values agree. Explicit arguments must meet their restrictions and agree with inferred values. For felt parameters, the comparison uses the required field and any known field or value of a symbolic argument. A type-variable wildcard must agree with signature values after its type is inferred. Direct index or integer arguments cannot be affine maps.
- A type in an included function or contract signature resolves under the `include.from` alias, even if its printed path matches a type in the caller.
