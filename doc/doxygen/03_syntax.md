...
- Felt-valued template arguments on `struct.type`, templated free-function calls, and `verif.include` may be integer literals, felt constants, or symbols.
  ...
  The `?` wildcard is valid only for a `poly.tvar` restriction and defers its concrete type to a later type-inference pass. Symbolic arguments must resolve to an enclosing template binding or a constant global; unknown, mutable-global, and wrong-kind symbols are rejected. Type-valued arguments on function calls and contract inclusions resolve nested symbols at the call or inclusion site. A nested `poly.tvar` is accepted once its parameter reference resolves, even when its concrete type remains deferred.
