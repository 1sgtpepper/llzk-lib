...
- Felt constants and integer arguments can instantiate parameters on `struct.type`, templated free-function calls, and `verif.include`. A `struct.member` with a fieldless felt restriction accepts any felt field. For a fielded restriction, a fieldless felt constant or integer is materialized in the required field, while an explicitly fielded constant or symbol with a known field must name that same field; an untyped `struct.member` cannot establish a required field. If a call or inclusion signature independently infers the same parameter, all known fields and concrete values must also agree. Symbolic arguments in all three forms must resolve to an enclosing template binding or a constant global; unknown, mutable-global, and wrong-kind symbols are rejected.
