...
- Felt arguments are valid template arguments for parameterized struct types, free-function calls, and `verif.include` contract inclusions. A fieldless required felt accepts a fieldless or explicitly fielded actual value; a fielded required felt accepts only the same explicitly fielded actual value.
