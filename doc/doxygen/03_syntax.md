...
- A templated `function.call` or `verif.include` may omit arguments only when every parameter appears in the target signature. Explicit arguments must meet their restrictions and agree with inferred values. Direct index or integer arguments cannot be affine maps.
